...
def setup_batch_esp_calculator(
    ...
    - `calculate_esp_batch(atom_stacks)`: list of `AtomStack`, one per output volume.
      Each `AtomStack` has shape `(B_ensemble, N, 3)`; all ensemble members are
      accumulated into that volume using its occupancies.
    - `calculate_esp_batch_from_coords(coords_batch, bfactors, atomic_numbers, occupancies)`:
    ...
    def calculate_esp_batch(atom_stacks: list[AtomStack]):
        """
